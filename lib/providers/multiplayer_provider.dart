...
class MultiplayerProvider with ChangeNotifier {
  ...
  Future<bool> kickPlayer(String targetPlayerId) async {
    try {
      final success = await _multiplayerService.kickPlayer(targetPlayerId);
      if (!success) {
        _errorMessage = 'Failed to kick player';
        notifyListeners();
      }
      return success;
    } catch (e) {
      _errorMessage = 'Failed to kick player: $e';
      notifyListeners();
      return false;
    }
  }

  /// Start game (send startGame request to server)
  Future<bool> startGame() async {
    final success = await _multiplayerService.startGame();
    if (!success) {
      _errorMessage = 'فشل في بدء اللعبة';
      notifyListeners();
    }
    return success;
  }
